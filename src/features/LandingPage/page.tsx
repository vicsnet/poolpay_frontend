...
import { Menu, X, Wallet, Users, Clock, ShieldCheck, Zap, TrendingUp, Handshake, ArrowRight, DollarSign, Calendar, Lock, ChevronDown, ChevronUp } from 'lucide-react';
import { ConnectButton } from '@rainbow-me/rainbowkit';

// Use a clean, professional color palette (Switched to deeper blue for vibrancy and professionalism)
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center py-4 md:justify-start md:space-x-10">
          ...
          <div className="hidden md:flex items-center justify-end md:flex-1 lg:w-0">
            <button className={`${primaryColor} ${primaryBgHover} text-white font-semibold py-2 px-4 rounded-xl shadow-lg hover:shadow-xl transition duration-300 transform hover:scale-105`}>
              Start a Pool Today <ArrowRight className="inline h-4 w-4 ml-1" />
            </button>
          </div>
        </div>
      </div>
          ...
          <div className="rounded-lg shadow-2xl ring-1 ring-black ring-opacity-5 bg-white divide-y-2 divide-gray-50">
            <div className="pt-5 pb-6 px-5">
              <div className="flex items-center justify-between">
                <div>
                  <span className="text-xl font-extrabold text-gray-900">PoolPay</span>
                </div>
                <div className="-mr-2">
                  <button
                ...
                <nav className="grid gap-y-4">
                  {navItems.map((item) => (
                    <a
                      ...
                      href={item.href}
                      onClick={() => setIsOpen(false)}
                      className="-m-3 p-3 flex items-center rounded-lg hover:bg-gray-50 transition duration-150"
                    >
      ...
      <div className="lg:grid lg:grid-cols-12 lg:gap-8">
        <div className="sm:text-center md:max-w-2xl md:mx-auto lg:col-span-6 lg:text-left">
          ...
          <div className="mt-10 sm:flex sm:justify-center lg:justify-start space-y-4 sm:space-y-0 sm:space-x-4">
            <button className={`${primaryColor} ${primaryBgHover} flex items-center justify-center w-full sm:w-auto px-8 py-3 border border-transparent text-base font-medium rounded-xl text-white md:py-4 md:text-lg md:px-10 shadow-xl hover:shadow-2xl transition duration-300 transform hover:scale-[1.02]`}>
              Start Saving Now
            </button>
            <button className={`flex items-center justify-center w-full sm:w-auto px-8 py-3 border-2 border-blue-700 text-base font-medium text-blue-700 bg-white md:py-4 md:text-lg md:px-10 hover:bg-blue-50 transition duration-300`}>
              Watch Demo Video
            </button>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-white font-[Inter]">
      ...
      <main>
        ...
        <Testimonials />
        <FAQs />
        {/* Call to action section: using bg-blue-800 for extra pop */}
        <div className="bg-blue-800">
          <div className="max-w-7xl mx-auto py-16 px-4 sm:px-6 lg:px-8">
            <div className="text-center">
              <h2 className="text-3xl font-extrabold text-white sm:text-4xl">
                Ready to Pool Your Pay?
              </h2>
              <p className="mt-4 text-xl text-blue-200">
                Join thousands achieving their financial goals faster and safer than ever before.
              </p>
              <div className="mt-8 flex justify-center">
                <button className="bg-white text-blue-800 font-bold py-3 px-8 rounded-xl shadow-2xl hover:bg-gray-100 transition duration-300 transform hover:scale-[1.05] flex items-center">
                  Create Your First Pool <ArrowRight className="h-5 w-5 ml-2" />
                </button>
              </div>
            </div>
          </div>
